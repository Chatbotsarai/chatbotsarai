...
def login_to_x(context, page):
    """Loguje do platformy X, używając zapisanego stanu lub logowania manualnego."""
    ...
    try:
        if os.path.exists(storage_state_path):
            ...
            if "login" not in page.url:
                print(f"✅ Zalogowano z zapisanego stanu! URL: {page.url}")
                return
            else:
                print("ℹ️ Zapisany stan sesji jest nieważny, próbuję logowania manualnego")
        ...
        print(f"✅ Zalogowano! URL: {page.url}")

        context.storage_state(path=storage_state_path)
        print(f"ℹ️ Stan sesji zapisany do {storage_state_path}")
